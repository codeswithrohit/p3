...
const Orders = () => {
        ...
        const unsubscribe = firebase.auth().onAuthStateChanged(async (user) => {
            if (user) {
                try {
                    ...
                    setLoading(false); // Handle error, stop loading
                }
            } else {
    ...
    useEffect(() => {
        const fetchBookingDetails = async () => {
            ...
            try {
                ...
                const bookingRef = db.collection('Orders')
                    .where('user.uid', '==', userData.uid); // Use where clause

                const snapshot = await bookingRef.get();

                if (snapshot.empty) {
                    console.log('No matching orders.');
                    setLoading(false); // No orders found, stop loading
                    return;
                }
  ...
  <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
    <div class="mx-auto max-w-5xl">
      ...
      <div key={order.id} class="mt-6 flow-root sm:mt-8">
        <div class="divide-y divide-gray-200 dark:divide-gray-700">
          <div class="flex flex-wrap items-center gap-y-4 py-6">
            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
              <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Order ID:</dt>
              <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">
                <a href="#" class="hover:underline">{order.orderId}</a>
              </dd>
            </dl>

            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
              <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Date:</dt>
              <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white"> {order.createdAt.toDate().toLocaleDateString()}</dd>
            </dl>

            <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
              <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Price:</dt>
              <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">₹{order.subtotal}</dd>
            </dl>
            ...
            <div class="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
              ...
              <a   href={`/order?orderId=${order.orderId}`}  class="w-full inline-flex justify-center rounded-lg  border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">View details</a>
            </div>
          </div>
